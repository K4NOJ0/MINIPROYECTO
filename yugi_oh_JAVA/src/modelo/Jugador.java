...
public class Jugador {
    ...
    public LinkedList<Carta> getManoLinked() {
        return mano;
    }

    public Stack<Carta> getMazoStack() {
        return mazo;
    }
    ...
    public String recibirDano(int dano) {
        if (wabokuActivo) {
            wabokuActivo = false;
            return nombre + " bloqueo el daño con Waboku ";
        }
    ...
    public String robarCarta() {
        if (mazo.isEmpty()) {
            throw new RuntimeException(new MazoVacioException(
                    nombre + " intento robar carta de un mazo vacío."));
        }
        Carta carta = mazo.pop();
        mano.addLast(carta);
        return nombre + " robo " + carta.getNombre();
    }
    ...
    public String sacrificarMonstruo() {
        if (this.getCampo().getZonaMonstruosLinked().isEmpty())
    ...
    public String destruirMonstruoEnemigo(Jugador enemigo) {
        if (enemigo.getCampo().getZonaMonstruosLinked().isEmpty())
