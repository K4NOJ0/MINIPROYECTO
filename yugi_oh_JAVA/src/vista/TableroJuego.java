...
public class TableroJuego extends JFrame {
    ...
    public interface TableroListener {
        ...
        void onGuardarPartida();   // NUEVO
        void onCargarPartida();    // NUEVO
    }
    ...
    private JPanel createBotones() {
        // 3 filas x 2 columnas = 6 botones
    ...
    public void limpiarSeleccionMano() { limpiarSeleccion(); }
    ...
    public DialogosJuego getDialogos() { return new DialogosJuego(this); }

    public void actualizarTablero() { revalidate(); repaint(); }
            ...
            if (esJugador) {
                cp.addMouseListener(new MouseAdapter() {
                    @Override public void mouseClicked(MouseEvent e) {
                        ...
                        if (r == JOptionPane.YES_OPTION) listener.onActivarTrampa(ct);
                    }
                });
            }
            cartas.add(cp);
